...
def osgiTestProject(p: Project, framework: ModuleID) = p
  ...
  .settings(
    ...
    unmanagedSourceDirectories in Test := List((baseDirectory in ThisBuild).value / "scalaTest" / "osgi" / "src"),
...
lazy val scalaTest = project
  ...
  .settings(
    ...
    /*   (baseDir / "scalaTest/files/lib").list.toSeq.filter(_.endsWith(".jar.desired.sha1")) */
    /*     .map(f => bootstrapDep(baseDir, "scalaTest/files/lib", f.dropRight(17))) */
    /* }, */
    // Two hardcoded dependencies in partest, resolved in the otherwise unused scope "test":
    /* libraryDependencies += bootstrapDep((baseDirectory in ThisBuild).value, "scalaTest/files/codelib", "code") % "test", */
    /* libraryDependencies += bootstrapDep((baseDirectory in ThisBuild).value, "scalaTest/files/speclib", "instrumented") % "test", */
    // no main sources
    ...
    testOptions in IntegrationTest += Tests.Setup { () =>
      ...
      copyBootstrapJar(cp, baseDir, "scalaTest/files/codelib", "code")
      copyBootstrapJar(cp, baseDir, "scalaTest/files/speclib", "instrumented")
    },
...
lazy val dist = (project in file("dist"))
  ...
  .settings(
    ...
    mkQuick := Def.task {
      val cp = (fullClasspath in IntegrationTest in LocalProject("scalaTest")).value
...
commands += Command("partest")(_ => PartestUtil.partestParser((baseDirectory in ThisBuild).value, (baseDirectory in ThisBuild).value / "scalaTest")) { (state, parsed) =>
  ("scalaTest/it:testOnly -- " + parsed) :: state
}

// Watch the test files also so ~partest triggers on test case changes
watchSources ++= PartestUtil.testFilePaths((baseDirectory in ThisBuild).value, (baseDirectory in ThisBuild).value / "scalaTest")
